	...
	define(function () {
		...
		css = {
			...
			load: function (name, req, load) { //, config (not used)
				...
				name = name.match(/\.(css|less|scss)$/i) ? name : name + '.css';
/*
				var url;
				if(name.match('^~')) {
					url = location.pathname + (location.pathname.substr(-1) == '/' ? '' : '/') + name.substr(1);
				} else {
					url = req.toUrl(name);
				}

				// Add random number to beginning for cache-busting
				if(isNaN(url[1])) {
	        url = (url[0] == '/' ? ('/' + cacheBuster) : (cacheBuster + '/')) + url;
	      }
*/
